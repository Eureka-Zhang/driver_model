...
- Lateral channels are generated by replaying each driver's original residual jitter.
  ...
  cleaned ``segment_*.csv`` and/or ``driving_data.csv`` under that tree, while scenario
  CSVs still come from ``--data_dir``.
...
def main():
    ...
    if not pool_paths and lat_drv and args.lateral_mode == "original_jitter":
        print(
            "[WARN] lateral_pool_driver={}: no lateral pool CSVs "
            "(segment_*.csv or following driving_data.csv) under {}; lateral jitter may be empty.".format(
                lat_drv, pool_root
            )
        )
